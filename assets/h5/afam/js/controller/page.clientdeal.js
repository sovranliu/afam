...
$Controller.methods = function() {
	return {
		...
		refresh:function() {
			...
			$$.getJSON(S_DOMAIN + '/afam/rest/client/deal', {"aid":aId}, function(resp) {
				if(resp.code < 0) {
					bridge('window').call('tip', resp.msg || '客户交易服务错误');
					return;
				}
				var data = resp.data;
				var html = null;
				html = Template7.templates.template_deal(data['application']);
				$$('#application_list').html(html);
				html = Template7.templates.template_deal(data['deal']);
				$$('#deal_list').html(html);
				html = Template7.templates.template_deal(data['transfer']);
				$$('#transfer_list').html(html);
				html = Template7.templates.template_deal(data['dx']);
				$$('#dx_list').html(html);
			});
		},
